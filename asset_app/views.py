...
class Loan_assetListFilterView(generic.ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.kwargs['task'] == "late":
            ...
            context["loab_title"] = "Udlån oversigt for {} for følgende dato {}".format(self.kwargs['loc_name'], datetime.datetime.strptime(self.kwargs['return_date'], "%Y-%m-%d").strftime("%d/%m/%Y"))
        elif self.kwargs['task'] == "monthPlus":
            context["loab_title"] = "Udlån oversigt for {} der skal afleveres efter den {}".format(self.kwargs['loc_name'], datetime.datetime.strptime(self.kwargs['return_date'], "%Y-%m-%d").strftime("d/%m/%Y"))
...
class LocationLaptopListView(generic.ListView):
    ...
    def get_queryset(self):
        ...
        queryset = super().get_queryset().filter(room__location__name=location).filter(may_be_loaned=True).exclude(is_loaned=True).order_by('name')
        return queryset
