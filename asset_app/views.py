...
class AssetCaseListView(generic.ListView):
    ...
    def set_solved_true(self, request, asset_case_id, pk, **kwargs):
    ...
    def set_solved_false(self, request, asset_case_id, pk, **kwargs):
        item = models.Assetcase.objects.get(pk=asset_case_id)
        item.solved = False
...
class Dashboard(generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['bundelReservations'] = models.Bundle_reservation.objects.filter(returned=False).filter(return_date > context_entry_inspection_time).order_by('return_date')
        context['loan_assets'] = models.Loan_asset.objects.filter(returned=False).filter(return_date > context_entry_inspection_time).order_by('return_date')
